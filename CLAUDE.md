...
- **No state mutation in `forward`.** Device is inferred from the input. `dilated_positions` is a non-persistent buffer, not a plain attribute. Together these give 0 graph breaks under `torch._dynamo.explain`; `torch.compile(fullgraph=True)` matches eager bit-for-bit. Check `explain` after touching `forward`.
